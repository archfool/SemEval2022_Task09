...
def rule_for_qa(dataset):
    ...
    if os.path.exists(u'/media/archfool/data'):
        ...
        for idx in range(len(qa_df)):
            qa_df['pred_answer'].iloc[idx] = get_answer_by_rule(qa_df.iloc[idx])
...
if __name__ == '__main__':
    ...
    # rule_result['score'] = rule_result.apply(lambda r: 1 if str(r['answer']) == str(r['pred_answer']) else 0, axis=1)
