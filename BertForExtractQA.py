...
class BertForExtractQA(BertPreTrainedModel):
    ...
    def forward(
        ...
        if extract_label is not None:
            ...
            loss = loss_fct(logits.reshape([-1, logits.size()[-1]]).contiguous(),
                            extract_label.reshape([-1]).contiguous())

        if not return_dict:
            return (logits)

        return ExtractQAModelOutput(
            loss=loss,
            logits=logits,
...
@dataclass
class ExtractQAModelOutput(ModelOutput):
    """
